...
# Runs Catalyst server on port 4001 with debug mode outside Zenflow
...
if lsof -Pi :4001 -sTCP:LISTEN -t >/dev/null ; then
    ...
    echo "Kill the existing process with: kill \$(lsof -t -i:4001)"
    exit 1
fi
...
CATALYST_DEBUG=1 script/comserv_server.pl -p 4001 -r
