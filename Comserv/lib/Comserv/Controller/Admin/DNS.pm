...
__PACKAGE__->config(namespace => 'admin/dns');
...
sub _allowed_zones {
    ...
    eval {
        ...
        for my $sd (@sitedomains) {
            ...
            my $is_exact_zone = ($d eq $zone);
            push @domains, $zone if $is_exact_zone;
        }
    };
...
sub add_record :Path('add_record') :Args(1) {
    ...
    return $c->response->redirect($c->uri_for('/admin/dns/zone/' . $zone_name))
    ...
    $c->response->redirect($c->uri_for('/admin/dns/zone/' . $zone_name));
}
