...
sub Author_id :Path('/ENCY/Author') :Args(1) {
    ...
    $c->stash(
        ...
        is_admin  => $is_admin,
        is_editor => $is_editor,
        ency_ai_prompt => 'Return ONLY a raw JSON object with these fields for the named author/practitioner: full_name (Last, First format — use the name exactly as it appears in professional directories), credentials (ALL degrees in order: undergraduate then professional, e.g. "BSc, ND" or "MD, PhD" — do NOT omit undergraduate degrees), specialty (specific clinical specialties separated by commas, e.g. "Naturopathic Oncology, Traditional Chinese Medicine, Integrative Cancer Care"), born_year (4-digit year ONLY if reliably known — omit field if uncertain), died_year (4-digit year ONLY if deceased and confirmed — omit if living or unknown), nationality (COUNTRY of citizenship/practice — be precise: "Canadian" not "American" for Canadian practitioners), affiliation (PRIMARY institution(s) where they practiced or taught — be specific, include city/province/state, e.g. "Boucher Institute of Naturopathic Medicine, New Westminster BC"), url (official biography page, institutional profile, or Wikipedia — verify URL is likely real before including), bio (write 3-4 detailed paragraphs covering: 1) Education and training — all degrees, institutions, graduation years; 2) Clinical career and specialties — what they treated, where they practiced; 3) Publications and major works — list book titles and editions; 4) Teaching philosophy and legacy — students influenced, institutions founded or led; 5) Relationship to herbal/naturopathic medicine and this reference system if known), notes (specific relevance to this encyclopedia — which reference numbers they authored, which herb/disease entries they informed, any direct quotes or passages from their work used in this system)',
