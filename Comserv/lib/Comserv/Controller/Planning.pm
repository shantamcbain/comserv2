        ...
        eval {
            ...
            if ($uid) {
                my @usr_rows = $c->model('DBEncy')->resultset('UserSiteRole')->search(
                    ...
                    { join => 'site', columns => [{ site_name => 'site.name' }], distinct => 1 }
                )->all;
                @ap_all_sitenames = sort grep { $_ } map { eval { $_->get_column('site_name') } || '' } @usr_rows;
            }
        };
        @ap_all_sitenames = ($sitename) unless @ap_all_sitenames;
