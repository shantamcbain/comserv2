...
sub transcribe :Local :Args(0) {
    ...
    # uploads of the same original name never collide on disk.
    require POSIX;
    ...
    unless ($python_bin) {
        ...
        if ($source_type eq 'nfs') {
            ...
            if ($dispatch_ok) {
                ...
                # The audio file is already saved to NFS + its File DB row is
                # created above, so report the upload as confirmed immediately.
                # Transcription continues in the background and the client polls
                # for the transcript separately.
                $c->response->body(encode_json({
                    success      => JSON::true,
                    uploaded     => JSON::true,
