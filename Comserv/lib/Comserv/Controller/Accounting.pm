...
sub accounting_dbs :Path('/Accounting/admin/databases') :Args(0) {
    ...
    for my $site (@sites) {
        ...
        if ($rec && $rec->status eq 'active') {
            ...
            # ACCPG Ph.1 — per-site accounting schema version check
            if ($row->{db_ok}) {
                my $ver = Comserv::Util::Accounting::Migrator->instance
                              ->schema_version_status($c, $sn);
                $row->{schema_version}  = $ver->{version};
                $row->{schema_state}    = $ver->{state};
                $row->{schema_expected} = $ver->{expected};
            }
        }
...
sub migrate_to_pg :Path('/Accounting/migrate_to_pg') :Args(0) {
    ...
    my ($log, $errors) = Comserv::Util::Accounting::Migrator->instance
        ->migrate_site($c, $sitename, $self->_schema($c));

    unless (defined $log) {
        # $errors carries the failure reason when the PG database is unreachable
