...
sub index :Path :Args(0) {
    ...
    $self->logging->log_with_details($c, __FILE__, __LINE__, 'Enter in index');

    # Get the current site name from the session
    my $current_site_name = $c->session->{SiteName};

    # Log the current site name
    $self->logging->log_with_details($c, __FILE__, __LINE__, "Got current site name $current_site_name");
