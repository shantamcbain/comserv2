...
sub index :Path :Args(0) {
    ...
    try {
        ...
        $c->stash(
            ...
            debug_msg => "Error occurred in ProxyManager controller index action"
        );
        return;
    };
}
    ...
    try {
        ...
        unless ($res->is_success) {
            ...
            $c->stash(
                ...
                technical_details => 'API request failed: ' . $error_details,
                action_required => 'Please check that the Nginx Proxy Manager is running and accessible, and that your API key is valid.',
                debug_msg => "Failed to create proxy in create_proxy action"
            );
            return;
        }
        ...
        $c->stash(
            ...
            technical_details => 'Exception: ' . $error_message,
            action_required => $action_required,
            debug_msg => "Exception caught in create_proxy action"
        );
        return;
    };
}
