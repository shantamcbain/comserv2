...
sub auto :Private {
    ...
    eval {
        ...
        # Read build version baked into the image (or generated by deploy)
        my $version_file = $c->config->{home} . '/version.json';
        if (-f $version_file) {
            my $app_version = eval {
                open my $vf, '<', $version_file or die;
                local $/; my $json = <$vf>; close $vf;
