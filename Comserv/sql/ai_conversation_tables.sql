...
-- Migration: add project_id if ai_conversations already exists without it
-- These ALTER TABLE statements are safe to run even if run multiple times
-- because they use IF NOT EXISTS (MySQL 8.x) or can be wrapped in a stored procedure.
-- For MySQL 5.x compatibility, handle at application level via Admin controller.
